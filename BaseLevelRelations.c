/****************************************************************
 *
 * $Source: /pv/CvsTree/pv/gen/src/prg/methods/FC2D_ANGIO/BaseLevelRelations.c,v $
 *
 ...
 * All Rights Reserved
 *
 * $Id: BaseLevelRelations.c,v 1.6.2.8 2006/01/03 09:03:26 mawi Exp $
 *
 ****************************************************************/

static const char resid[] = "$Id: BaseLevelRelations.c,v 1.6.2.8 2006/01/03 09:03:26 mawi Exp $ (C) 2002-2005 Bruker BioSpin MRI GmbH";
...
void SetFrequencyParameters( void )
{
  ...
  /* From FLASHubc */
  ACQ_O2_list_size = ACQ_size[1];
  ParxRelsParRelations("ACQ_O2_list_size", Yes);
  MRT_RFSpoilFreqList(RFSpoilPhi,ACQ_O2_list_size,ACQ_O2_list,(RFSpoilDelay-0.01));
...
//                   PVM_ReadGradient,		/* t5 */
                   ...
                   ReadSpoilerStrength,		/* t9, same as FLASHubc t6 */
...
void SetPpgParameters( void )
{
  ...
  D[0]  = (PVM_RepetitionTime - PVM_MinRepetitionTime + PVM_InterGradientWaitTime) / 1000.0;
  D[2]  = (PVM_EchoTime - PVM_MinEchoTime + PVM_InterGradientWaitTime)/ 1000.0;
  D[3]  = (PVM_FCtau - (PVM_RampTime + PVM_InterGradientWaitTime))/ 1000.0;
  D[5]  = (PVM_FCtau - (PVM_RampTime + PVM_InterGradientWaitTime))/ 1000.0;
  D[6]  = (PVM_FCtau - (PVM_RampTime + PVM_InterGradientWaitTime))/ 1000.0;
  D[7] = 0;//D[7]  = (ReadSpoilerDuration - PVM_2dPhaseFCGradientTime)/1000.0;
  D[8]  = CFG_AmplifierEnable()/1000.0; 
  D[9]  = (SliceSpoilerDuration - (PVM_RampTime + PVM_InterGradientWaitTime))/1000.0;
  D[10] = (PVM_ExSliceRephaseTime - PVM_RampTime) / 1000.0;
  D[11] = (PVM_ReadDephaseTime - PVM_RampTime) / 1000.0;
  D[12] = (ReadSpoilerDuration - PVM_ReadDephaseTime)/1000.0;
  D[13] = (PVM_RiseTime) / 1000.0; // this used to be D[3] in FLASHubc
  D[14] = (PVM_RampTime) / 1000.0; // this used to be D[4] in FLASHubc
  D[16] = (SliceSpoilerDuration - PVM_RiseTime)/1000.0; // this used to be D[6] in the FLASHubc code
  D[19] =  RFSpoilDelay/1000 - 2e-6;
  D[25] = 1.0e-3*RFSpoilerdelay;
  D[30] = (PVM_RampTime+PVM_InterGradientWaitTime) / 1000.0;
